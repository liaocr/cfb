...
## 2026-09-24 追加（v11.8 整理）

| 文件 | 性质 |
|---|---|
| `at-birth-interception.md` | birth 模式（出生即拦截）最初的架构规范（2026-09-16，27/27 测试时期）；现行架构见 `../ARCHITECTURE.md` |
| `ARCHITECTURE-CONSOLIDATED.md` | v8 可逆上下文分层的架构整合（覆盖姊妹仓库的整体设计，非本包现状） |

v11.8 同时移除了若干已退役功能的代码（distill/rules 模式、证据视图、编译排队、快照镜像、cover.json 水位），
本目录中描述这些功能的历史报告保持原样；代码可从提交 `e818cff` 取回，差异说明见根目录 `CHANGELOG.md` 的 v11.8 条目。
